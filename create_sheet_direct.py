#!/usr/bin/env python3
"""
Create Google Sheet directly using service account or application default credentials
"""
...
def create_sheet_via_sheets_api():
    """Create Google Sheet using google-api-python-client"""
    try:
        from googleapiclient.discovery import build
        from google.auth import default
    except ImportError:
        print("❌ Missing google-api-python-client. Install: pip3 install google-api-python-client google-auth")
        return None
    
    # Get credentials (Application Default Credentials)
    try:
        credentials, project = default(scopes=['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive'])
        print(f"✅ Using credentials from: {type(credentials).__name__}")
    except Exception as e:
        print(f"❌ Could not find credentials: {e}")
        print("\nTry one of:")
    ...
    return sheet_id, sheets_service, sheet_id
...
def main():
    ...
    # Create sheet
    result = create_sheet_via_sheets_api()
    if not result:
        return
    
    sheet_id, sheets_service, _ = result
    ...
    print(f"\n⏭️  Next: Update config.yaml with this Sheet ID")
